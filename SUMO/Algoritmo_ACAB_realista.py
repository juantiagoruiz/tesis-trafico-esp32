...
tiempo_seteado_TFL5 = 0


tiempo_lastvehicle_TFL1 = {"EO": 0.0, "NS": 0.0, "SN": 0.0,"DETECTION": 0.0}
tiempo_lastvehicle_TFL2 = {"EO": 0.0, "NS": 0.0, "SN": 0.0,"DETECTION": 0.0}
tiempo_lastvehicle_TFL3 = {"EO": 0.0, "NS": 0.0, "SN": 0.0,"DETECTION": 0.0}
tiempo_lastvehicle_TFL4 = {"EO": 0.0, "NS": 0.0, "SN": 0.0,"DETECTION": 0.0}
tiempo_lastvehicle_TFL5 = {"EO": 0.0, "NS": 0.0, "SN": 0.0,"DETECTION": 0.0}
...
def set_TFL(TFL,E2_EO1,E2_EO2,E2_EO3,E2_NS,E2_SN,lanes_EO,lanes_NS,tiempo_seteado,lastvehicle_times):
    accuracy = 0.707   # Precision de 70.7%
    false_positive = 0.0254 # Falsos positivos de 2.54%
    total_vehicles_EO = 0
    total_vehicles_NS = 0
    total_vehicles_SN = 0
    detected_vehicles_EO = 0
    detected_vehicles_NS = 0
    detected_vehicles_SN = 0
	
    if traci.trafficlight.getPhase(TFL) == 1 or traci.trafficlight.getPhase(TFL) == 3 or traci.trafficlight.getPhase(TFL) == 5:
        lastvehicle_times["EO"] = traci.simulation.getCurrentTime()
        lastvehicle_times["SN"] = traci.simulation.getCurrentTime()
        lastvehicle_times["NS"] = traci.simulation.getCurrentTime()
    
    if ((traci.simulation.getCurrentTime() -lastvehicle_times["DETECTION"]) >= 1125): # una deteccion cada 1125 ms
        tiempo_seteado = 0
        if (TFL == "1686155185"):
            print("DETECTION")
		
    if tiempo_seteado == 0:
        total_vehicles_EO = traci.lanearea.getLastStepVehicleNumber(E2_EO1)+traci.lanearea.getLastStepVehicleNumber(E2_EO2)+traci.lanearea.getLastStepVehicleNumber(E2_EO3)
        if (TFL == "1686155185"):
                print(f"Vehiculos EO: {total_vehicles_EO:.2f}")
        
        total_vehicles_NS = traci.lanearea.getLastStepVehicleNumber(E2_NS)
        if TFL == "1686155185":
            print(f"Vehiculos NS: {total_vehicles_NS:.2f}")       
        
        total_vehicles_SN = traci.lanearea.getLastStepVehicleNumber(E2_SN)
        if TFL == "1686155185":
                print(f"Vehiculos SN: {total_vehicles_SN:.2f}")	

        for _ in range(total_vehicles_EO):
            if random.random() <= accuracy:
                detected_vehicles_EO += 1
        if random.random() <= false_positive:
            detected_vehicles_EO += 1
        if (TFL == "1686155185"):
            print(f"Vehiculos Detectados EO: {detected_vehicles_EO:.2f}")
        
        for _ in range(total_vehicles_NS):
            if random.random() <= accuracy:
                detected_vehicles_NS += 1
        if random.random() <= false_positive:
            detected_vehicles_NS += 1
        if (TFL == "1686155185"):
            print(f"Vehiculos Detectados NS: {detected_vehicles_NS:.2f}")
        
        for _ in range(total_vehicles_SN):
            if random.random() <= accuracy:
                detected_vehicles_SN += 1
        if random.random() <= false_positive:
            detected_vehicles_SN += 1
        if (TFL == "1686155185"):
            print(f"Vehiculos Detectados SN: {detected_vehicles_SN:.2f}")

        if detected_vehicles_EO > 0:
            lastvehicle_times["EO"] = traci.simulation.getCurrentTime()            
			
        if detected_vehicles_NS > 0:
            lastvehicle_times["NS"] = traci.simulation.getCurrentTime()
			
        if detected_vehicles_SN > 0:
            lastvehicle_times["SN"] = traci.simulation.getCurrentTime()

    
        if traci.trafficlight.getPhase(TFL) == 0:       
            time_since = traci.simulation.getCurrentTime() - lastvehicle_times["NS"]
            if time_since >= 4499: #4 detecciones sin vehiculos
                traci.trafficlight.setPhase(TFL,1)
                print("No Vehiculos NS")
            tiempo_seteado = 1
        
        if traci.trafficlight.getPhase(TFL) == 2:            
            time_since = traci.simulation.getCurrentTime() - lastvehicle_times["SN"]
            if time_since >= 4499: #4 detecciones sin vehiculos
                traci.trafficlight.setPhase(TFL,3)
                print("No Vehiculos SN")
            #print(total_vehicles_SN)
            tiempo_seteado = 1

        if traci.trafficlight.getPhase(TFL) == 4:            
            time_since = traci.simulation.getCurrentTime() - lastvehicle_times["EO"]
            if time_since >= 4499: #4 detecciones sin vehiculos
                traci.trafficlight.setPhase(TFL,5)
                print("No Vehiculos EO")
            tiempo_seteado = 1
			
        lastvehicle_times["DETECTION"] = traci.simulation.getCurrentTime()
        return tiempo_seteado
    
def set_TFL2(TFL,E2_EO1,E2_EO2,E2_EO3,E2_NS1,E2_NS2,E2_SN1,E2_SN2,lanes_EO,lanes_NS,tiempo_seteado,lastvehicle_times):
    accuracy = 0.707   # Precision de 70.7%
    false_positive = 0.0254 # Falsos positivos de 2.54%
    total_vehicles_EO = 0
    total_vehicles_NS = 0
    ...
    if tiempo_seteado == 0:
        ...
        total_vehicles_NS = traci.lanearea.getLastStepVehicleNumber(E2_NS1) + traci.lanearea.getLastStepVehicleNumber(E2_NS2)
        if TFL == "1056968883a":
            print(f"Vehiculos NS: {total_vehicles_NS:.2f}")       
        
        total_vehicles_SN = traci.lanearea.getLastStepVehicleNumber(E2_SN1) + traci.lanearea.getLastStepVehicleNumber(E2_SN2)
        if TFL == "1056968883a":
                print(f"Vehiculos SN: {total_vehicles_SN:.2f}")	
...
while traci.simulation.getMinExpectedNumber() > 0 and  traci.simulation.getTime() < 2000:
    ...
    tiempo_seteado_TFL1 = set_TFL("1738355191","e2_0","e2_1","e2_2","e2_15","e2_16",2,1,tiempo_seteado_TFL1,tiempo_lastvehicle_TFL1)
    tiempo_seteado_TFL2 = set_TFL("1951912875","e2_3","e2_4","e2_5","e2_13","e2_14",2,1,tiempo_seteado_TFL2,tiempo_lastvehicle_TFL2)
    tiempo_seteado_TFL3 = set_TFL2("1056968889","e2_6","e2_7","e2_8","e2_9","e2_10","e2_11","e2_12",1.5,1.5,tiempo_seteado_TFL3,tiempo_lastvehicle_TFL3)
    tiempo_seteado_TFL4 = set_TFL2("1056968883","e2_21","e2_22","e2_27","e2_23","e2_24","e2_25","e2_26",1.5,1,tiempo_seteado_TFL4,tiempo_lastvehicle_TFL4)
    tiempo_seteado_TFL5 = set_TFL("1686155185","e2_19","e2_20","e2_28","e2_18","e2_17",1.5,1,tiempo_seteado_TFL5,tiempo_lastvehicle_TFL5)
    step_count += 1
...
Queue_HistoryDF.to_csv('C:/sumo_tesis/Queue_HistoryDF1.csv')
...
# Se exportan los datos como archivo csv
vehicle_dataDF.to_csv('C:/sumo_tesis/vehicle_dataDF1.csv')
...
# Se exportan los datos como archivo csv
waiting_dataDF.to_csv('C:/sumo_tesis/waiting_dataDF1.csv')
...
# Se exportan los datos como archivo csv
emissions_dataDF.to_csv('C:/sumo_tesis/emissions_dataDF1.csv')
